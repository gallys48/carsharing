...
def test_create_car_success(db_connection):
    with db_connection.cursor() as cur:
        ...
        cur.execute("SELECT vin, maker, model, color, year, daily_rate, is_available, sys_status FROM carsharing.cars;")
        result = cur.fetchone()
        assert result == ('123456789', 'Toyota', 'Camry', 'Red',2020, 2000.00, True, 1)

# Создание ТС с уже существующим vin
def test_create_car_duplicate(db_connection):
    with db_connection.cursor() as cur:
        cur.execute("CALL carsharing.create_car('123', 'Toyota', 'Camry');")
        db_connection.commit()
        with pytest.raises(psycopg.errors.RaiseException) as exc:
            cur.execute("CALL carsharing.create_car('123', 'Toyota', 'Corolla');")
        assert "уже есть в базе" in str(exc.value)

# Полное обновление информации о ТС
def test_update_car_all_fields(db_connection):
    with db_connection.cursor() as cur:
        cur.execute("CALL carsharing.create_car('001', 'Toyota', 'Camry', 'Red', 2020, 2000, true);")
        cur.execute("SELECT id FROM carsharing.cars WHERE vin = '001';")
        ...
        cur.execute("SELECT vin, maker, model, color, year, daily_rate, is_available FROM carsharing.cars WHERE id = %s;", (car_id,))
        result = cur.fetchone()
        assert result == ('002', 'Honda', 'Accord', 'Green', 2021, 2500, False)

# Частичное обновление информации о ТС
def test_update_car_partial(db_connection):
    with db_connection.cursor() as cur:
        cur.execute("CALL carsharing.create_car('010', 'Toyota', 'Corolla', 'Red', 2019, 1500, true);")
        cur.execute("SELECT id FROM carsharing.cars WHERE vin = '010';")
        car_id = cur.fetchone()[0]
...
def test_delete_car_success(db_connection):
    with db_connection.cursor() as cur:
        cur.execute("CALL carsharing.create_car('123', 'Toyota', 'Camry');")
        cur.execute("SELECT id FROM carsharing.cars WHERE vin='123';")
        car_id = cur.fetchone()[0]
...
def test_delete_car_with_active_rental(db_connection):
    with db_connection.cursor() as cur:
        ...
        with pytest.raises(psycopg.errors.RaiseException) as exc:
            cur.execute("CALL carsharing.delete_car(%s);", (car_id,))
        assert "еще арендована" in str(exc.value)
...
def test_available_cars(db_connection):
    with db_connection.cursor() as cur:
        ...
        cur.execute("CALL carsharing.create_car('001', 'Toyota', 'Camry', 'Red', 2020, 2000, true);")
        cur.execute("CALL carsharing.create_car('002', 'Honda', 'Civic', 'Green', 2019, 1800, false);")
        cur.execute("CALL carsharing.create_car('003', 'BMW', 'X5', 'Black', 2021, 3000, true);")
